      ...
      return (
        <div className="artwork-list-item" key={artwork._id}>
          <Link
            ...
            to={`/gallery/inventory/${artwork._id}`}
          ></Link>
          <img
            src={artwork.imageUrl}
            alt={artwork.imgPublicId}
            className="artwork-list-item-thumbnail"
          />
          <div className="artwork-list-item-info">
            <span className="artwork-list-item-name">
              {artwork.artist.name}
            </span>
            , {artwork.title} {artwork.realisationYear}, Signed:{" "}
            {artwork.signed ? "yes" : "no"}, {artwork.materialsAndTechnique},{" "}
            {artwork.height +
              "x" +
              artwork.width +
              "x" +
              artwork.length +
              "(mm x mm x mm)"}
            , Stock number: {artwork.stockNumber}, Price: {artwork.price}€,
            ...
            <div className="gallery-content">
              <div className="flex-inventory-between">
                <div className="filter-box">
                  {/* implement later */}
                  Unique artworks / Edition and multiples / Products / Artist
                  books & Publications
                </div>
                <button
                  onClick={() => props.history.push("/gallery/inventory/new")}
                  className="btn-add-artwork"
                >
                  ADD A NEW ARTWORK
                </button>
              </div>
              <div className="flex-inventory-between ">
                ...
                <div className="inventory-sort">
                  <input
                    type="checkbox"
                    id="sort-by-value"
                    onChange={toggleSortPrice}
                    checked={sortByPrice}
                  />
                  <label className="label-button-like" htmlFor="sort-by-value">
                    SORT BY VALUE
                  </label>
                </div>
              </div>
