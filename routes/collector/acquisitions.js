...
router.get("/galleries", isAuthenticated, async (req, res, next) => {
  try {
    ...
    const data = await galleries.map((gallery) => ({
      id: gallery._id,
      name: gallery.name,
    }));
...
router.post("/request", isAuthenticated, async (req, res, next) => {
  ...
  if (data.artist && !data.suggestion) {
    ...
    const matchedArtist = await Artist.findOne({
  ...
  try {
    if (decision === "accept") {
      const updatedRequest = await Request.findOneAndUpdate(
        { _id: requestId, collector: collector._id },
        { offerStatus: "Accepted" },
        { new: true }
      );
      if (updatedRequest)
        return res.status(200).json({ success: true, decision: "accept" });
    }
    if (decision === "cancel") {
      const updatedRequest = await Request.findOneAndUpdate(
        { _id: requestId, collector: collector._id },
        { offerStatus: "Cancelled", status: "Confirmed" },
        { new: true }
      );
      if (updatedRequest)
        return res.status(200).json({ success: true, decision: "cancel" });
    }
    return;
  } catch (error) {
    console.log(error);
